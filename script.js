// ===============================
// script.js FINAL con sensibilidad calibrada agregada
// SIN CAMBIAR NADA EXTRA
// ===============================
...
// ===============================
// Sensibilidad Calibrada (A-B 85% / C 15%)
// ===============================
function sensibilidadCalibrada() {
  const r = Math.random();
  
  // A y B = 85% del total
  if (r < 0.85) {
    // A y B mezcladas internamente
    // valores más estables y controlados
    return rand(120,180); 
  }
...
// ===============================
// Lógica de botón según pantalla y sensibilidad
// ===============================
...
// Red Dot
// ===============================
function chooseRedDot(){ return Math.random()<0.05?rand(30,79):rand(80,200); }
...
function generate(withDpi){
  ...
  // 🔥 Usamos la nueva sensibilidad calibrada
  const general = sensibilidadCalibrada();
  
  const red = chooseRedDot();
  const x2 = clamp(sensibilidadCalibrada() + rand(-15,15),80,200);
  const x4 = clamp(sensibilidadCalibrada() + rand(-15,15),80,200);
