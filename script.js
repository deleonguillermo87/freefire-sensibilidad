...
// =============================
// Generador de botón (95 / 5)
// =============================
function generateShootButton() {
    const prob = Math.random() * 100;

    if (prob < 95) {
        // 95% → botón MEDIANO (35 a 65)
        return Math.floor(Math.random() * (65 - 35 + 1)) + 35;
    } else {
        // 5% → botón GRANDE (65 a 100)
...
document.getElementById("model").addEventListener("change", function () {
    ...
    // Generar botón usando la nueva lógica
    const shootButton = generateShootButton();

    resultBox.innerHTML = `
        <h3>Configuración recomendada:</h3>
        <p><b>Botón de disparo generado:</b> ${shootButton}</p>
    `;
});
