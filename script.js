...
// Función para limitar valores entre 0 y 200
function limitar(valor) {
    return Math.min(200, Math.max(0, valor));
}

// Mostrar valores al seleccionar teléfono
...
function calcularSensibilidad() {
    ...
    let general = limitar(((dpi / 10) * (tasa / 60)).toFixed(1));
    let puntoRojo = limitar(((dpi / 8) * (tasa / 60)).toFixed(1));
    let mira4x = limitar(((dpi / 12) * (tasa / 60)).toFixed(1));
